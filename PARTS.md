...
| 検索する文字列 | 意味 | 差し替え |
| --- | --- | --- |
| `thee3third@gmail.com` | メール送信先（設定済み） | 変更時に置換。`mailto:` とフォーム説明の両方に出現 |
...
> メール送信先 `thee3third@gmail.com` は **全ページ**（contactの2箇所／action-bar／各CTA）に出現します。変更する場合は必ず「全ファイル置換」で。
...
# メール送信先（現在 thee3third@gmail.com）。変更時の確認用
grep -rn "thee3third@gmail.com" *.html
